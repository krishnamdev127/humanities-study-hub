...
async function start(root){
 ...
 const ids=shuffle(pool).slice(0,n).map(q=>q.id); state.test={name:`Timed MCQ — ${subject||'All Subjects'}`,type:'quick',config:{subject,chapter_id:chapter,difficulty:diff,count:ids.length,duration_sec:duration},question_ids:ids,duration_sec:duration,total_marks:ids.length};
